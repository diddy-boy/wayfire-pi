...
ensure_candy_icon_theme() {
    # candy-icons (EliverLara) is a plain freedesktop icon theme — the
    # repo root itself *is* the theme (index.theme + apps/devices/
    # mimetypes/places/preferences/status dirs), no install.sh, no
    # build step. Just needs to land at ~/.local/share/icons/candy-icons.
    local target_dir="$HOME/.local/share/icons/candy-icons"
    ...
    # reason: keep whiptail's drawing off the stdout this function's
    # caller captures as the real return value.
    ui --title "wayfire-pi" --infobox "Installing the candy-icons icon theme..." 6 55 >/dev/tty 2>&1 || true
    ...
    if [[ "$result" == "0" && -d "${build_dir}/candy-icons" ]]; then
        rm -rf "$target_dir"
        mv "${build_dir}/candy-icons" "$target_dir"
    else
        result=1
    fi
    rm -rf "$build_dir"
    ...
    [[ -f "${target_dir}/index.theme" ]] || return 1
    echo "candy-icons"
    return 0
}

write_gtk_glass_theme() {
    # $1 skip_mactahoe yes/no (default no). When "yes", MacTahoe is not
    # installed or referenced at all — Minimal Plus's permanent,
    # intentional choice, made after testing showed MacTahoe's own
    # theme CSS was fighting this script's overrides (see the git
    # history around the light-background investigation). candy-icons
    # still applies on top for colour either way — this only affects
    # the GTK chrome/window theme itself.
    local skip_mactahoe="${1:-no}"
    ...
    if [[ "$skip_mactahoe" == "yes" ]]; then
        gtk_theme="Adwaita-dark"
        icon_theme=$(ensure_candy_icon_theme) || icon_theme="Papirus-Dark"
        result="skipped"
    elif gtk_theme=$(ensure_mactahoe_gtk_theme); then
        result="mactahoe"
        icon_theme=$(ensure_candy_icon_theme) || icon_theme="Papirus-Dark"
    else
        gtk_theme="Adwaita-dark"
...
/* Treeview/list content areas (file manager, file-open/save dialogs,
   any GTK3 list/tree widget) — dark glass, matching the rest of the
   theme, rather than the light Finder-style panes this used to force.
   !important throughout: the installed MacTahoe theme CSS is more
   specific than these plain selectors and was winning the cascade,
   which is why the earlier version of this rule (without !important)
...
FmMainWin .view,
FmMainWin treeview {
    background-color: rgba(24, 24, 37, 0.72) !important;
    background-image: none !important;
    color: #f3f3f3 !important;
}
...
treeview:selected,
.side-pane:selected {
    background-color: rgba(203, 166, 247, 0.40) !important;
    color: #ffffff !important;
}
...
remove_glass_theme_files() {
    ...
    rm -rf "$HOME"/.local/share/icons/candy-icons 2>/dev/null || true
...
build_labwc_minimal_rc() {
    # $1 corner radius in px (default 8 — labwc's own built-in default,
    # so calling this with no args leaves plain Minimal's look unchanged).
    # $2 dropShadows yes/no (default yes, matching the "retains default
    # server-side window drop shadows" behaviour already documented above).
    # $3 switcher_preview yes/no (default no) — adds a themed Alt-Tab
    # switcher with live window previews instead of labwc's bare list.
    # $4 maximized_decoration — labwc value for <maximizedDecoration>
    # (e.g. "titlebar"); empty (default) omits the tag, so labwc's own
    # default applies.
    # $5 keep_border yes/no — <keepBorder> value; empty (default) omits
    # the tag, so labwc's own default (yes) applies.
    # $6 themed_fonts yes/no (default no) — if yes, sets a consistent
    # sans 10 font across window titles, menus, and the OSD, rather than
    # leaving each context on whatever labwc falls back to individually.
    local corner_radius="${1:-8}"
    local drop_shadows="${2:-yes}"
    local switcher_preview="${3:-no}"
    local maximized_decoration="${4:-}"
    local keep_border="${5:-}"
    local themed_fonts="${6:-no}"
    local labwc_cfg_dir="${HOME}/.config/labwc"
...

    {
        ...
        if [[ "$switcher_preview" == "yes" ]]; then
            echo '  <core>'
            echo '    <windowSwitcher show="yes" preview="yes" outlines="no" />'
            echo '  </core>'
        fi
        ...
        echo '  <theme>'
        echo '    <name>Pix</name>'
        echo "    <cornerRadius>${corner_radius}</cornerRadius>"
        echo "    <dropShadows>${drop_shadows}</dropShadows>"
        [[ -n "$maximized_decoration" ]] && echo "    <maximizedDecoration>${maximized_decoration}</maximizedDecoration>"
        [[ -n "$keep_border" ]] && echo "    <keepBorder>${keep_border}</keepBorder>"
        if [[ "$themed_fonts" == "yes" ]]; then
            for place in ActiveWindow InactiveWindow MenuHeader MenuItem OnScreenDisplay; do
                echo "    <font place=\"${place}\">"
                echo '      <name>sans</name>'
                echo '      <size>10</size>'
                echo '    </font>'
            done
...
ensure_swaync_swayosd() {
    ...
    ui --title "wayfire-pi" --infobox "Installing ${pkgs[*]}...\n\nThis can take a minute or two — it's still working even if the terminal output pauses." 8 66
    sleep 1
    ...
    # swayosd's postinst enables its systemd units (the "Created
    # symlink..." line — fast) and then tries to *start* them
    # immediately as part of the install. There's no active graphical
    # session for swayosd-libinput-backend to attach to yet from a
    # terminal/whiptail install run, so that start attempt stalls
    # until it times out — apt looks hung but is really just waiting
    # that out. Since applying this tier already ends with "reboot or
    # logout/login required", the services come up fine on next login
    # regardless — so policy-rc.d below tells postinst scripts not to
    # start anything during this install. Enabling was never the slow
    # part, so this doesn't lose anything, and only applies if nothing
    # else already has a policy-rc.d in place (don't want to clobber
    # or remove one this script didn't create).
    local policy_rc_d="/usr/sbin/policy-rc.d"
    local had_policy_rc_d=0
    [[ -e "$policy_rc_d" ]] && had_policy_rc_d=1
    if [[ "$had_policy_rc_d" -eq 0 ]]; then
        sudo tee "$policy_rc_d" >/dev/null <<'EOF'
#!/bin/sh
exit 101
EOF
        sudo chmod +x "$policy_rc_d"
        # Belt-and-braces: if this function gets interrupted (Ctrl+C,
        # closed terminal) mid-install, a leftover policy-rc.d silently
        # blocks every service start system-wide until someone notices
        # — worse than the stall it's meant to fix. This trap clears it
        # on any exit path, not just the normal one below.
        trap 'sudo rm -f "/usr/sbin/policy-rc.d" 2>/dev/null' EXIT
    fi

    local install_result=0
    sudo apt-get install -y "${pkgs[@]}" || install_result=1
...
# ensure_pi_menu_icon — wf-panel-pi's smenu widget looks up a plain
# freedesktop icon name, "start-here", through whatever GTK icon theme
# is active; it has no Raspberry Pi logo baked into the widget itself.
# Stock Pi OS's own "PiXtrix" icon theme (confirmed present on a live
# Trixie box, see reset_gtk_theme above) ships a Raspberry-badged
# start-here icon under that exact name. candy-icons doesn't define one
# at all, so on Minimal Plus it falls through candy-icons' own
# Inherits= chain (breeze-dark, Adwaita, hicolor) to a generic fallback
# instead — the "default GNOME icon" symptom.
#
# Fix: copy PiXtrix's start-here icon into ~/.local/share/icons/
# hicolor — the universal fallback every conformant icon theme
# inherits from by spec (candy-icons' own index.theme lists hicolor
# last), so this fixes the menu icon regardless of which icon theme
# ends up active and survives future icon-theme changes without
# needing to patch each one individually. Best-effort: if PiXtrix
# isn't found at the expected path (e.g. a future Pi OS release renames
# it again the way Bookworm's PiXflat became Trixie's PiXtrix), this
# just silently does nothing rather than failing the whole tier.
ensure_pi_menu_icon() {
    local hicolor_dir="$HOME/.local/share/icons/hicolor"
    local found_any=0

    while IFS= read -r -d '' src; do
        # src looks like /usr/share/icons/PiXtrix/32x32/places/
        # start-here.png or .../scalable/places/start-here.svg — mirror
        # that same size/category path under our local hicolor override
        # rather than assuming one fixed size/format.
        local rel dest_dir
        rel="${src#/usr/share/icons/PiXtrix/}"
        dest_dir="${hicolor_dir}/${rel%/*}"
        mkdir -p "$dest_dir"
        cp -f "$src" "$dest_dir/" 2>/dev/null && found_any=1
    done < <(find /usr/share/icons/PiXtrix -iname 'start-here.*' -print0 2>/dev/null)
...
apply_tier() {
    ...
    if [[ "$tier" == "minimal_plus" ]]; then
        ...
        #
        # 16px corner radius (double labwc's built-in 8px) for a softer,
        # glassier window shape than plain Minimal. Keep an eye on this
        # alongside the opacity-seam warning above (build_labwc_
        # themerc_override) — Minimal Plus's titlebar alpha is already
        # lower than Minimal's, so a bigger radius has more seam to show.
        # Drop shadows on explicitly, tuned to the glass palette below
        # rather than labwc's default flat black. Also turns on the
        # previewed Alt-Tab switcher, a titlebar on maximized windows
        # (so they don't lose the glass chrome when maxed), an explicit
        # keepBorder, and a consistent sans-10 font across window
        # titles/menus/OSD.
        rm -f "$WAYFIRE_INI" "$WF_SHELL_INI"
        ensure_swaync_swayosd || true
        build_labwc_minimal_rc 16 yes yes titlebar yes yes
        build_labwc_autostart

        # 2. Install Glass GTK/Icon Theme
        #
        # Minimal Plus deliberately skips MacTahoe and stays on
        # Adwaita-dark: MacTahoe's own theme CSS was found to fight
        # this script's !important overrides (white treeview/menu
        # backgrounds), and staying on stock Adwaita-dark also keeps
        # Minimal Plus visually distinct from Average/Ludicrous and
        # avoids MacTahoe's multi-minute first-run build on a Pi.
        # candy-icons still applies here for colour.
        local theme_used
        theme_used=$(write_gtk_glass_theme "yes")
        ensure_pi_menu_icon || true
...
window.active.shadow.color: #1e1e2e 60
window.inactive.shadow.color: #1e1e2e 40
window.button.hover.bg.color: #cdd6f4 25
window.button.hover.bg.corner-radius: 13
EOF

        # 4. Extend the glass look to GTK3 tooltips and scrollbars —
        # same #1e1e2e/40% used for titlebars/menus above, so hovering a
        # tooltip or dragging a scrollbar doesn't suddenly land on an
        # opaque widget in the middle of an otherwise-glass desktop.
        local gtk3_cfg_dir="${HOME}/.config/gtk-3.0"
        mkdir -p "$gtk3_cfg_dir"
        cat > "${gtk3_cfg_dir}/gtk.css" <<EOF
tooltip {
...
scrollbar slider {
    background-color: rgba(205, 214, 244, 0.35);
    border-radius: 8px;
}

scrollbar slider:hover {
    background-color: rgba(205, 214, 244, 0.55);
}

scrollbar trough {
    background-color: rgba(30, 30, 46, 0.20);
}
EOF

        # 5. Best-effort glass styling for swaync notifications and the
        # swayosd volume/brightness OSD, so they don't stand out as flat
        # opaque boxes against the rest of the glass theme. Both are
        # gated on the binary being present and are non-fatal: swaync's
        # and swayosd's CSS class names have changed across versions
        # before, so if a given release doesn't pick these up, Minimal
        # Plus still applies fine without them.
        if command -v swaync >/dev/null 2>&1; then
            local swaync_cfg_dir="${HOME}/.config/swaync"
            mkdir -p "$swaync_cfg_dir"
            cat > "${swaync_cfg_dir}/style.css" <<EOF 2>/dev/null || true
...
EOF
        ...
        if command -v swayosd-server >/dev/null 2>&1; then
            local swayosd_cfg_dir="${HOME}/.config/swayosd"
            mkdir -p "$swayosd_cfg_dir"
            cat > "${swayosd_cfg_dir}/style.css" <<EOF 2>/dev/null || true
window {
    background-color: rgba(30, 30, 46, 0.85);
    color: #cdd6f4;
    border-radius: 16px;
...
EOF
        ...
        # 6. Glass Panel styling — a bit more transparent than the
        ...
        # 7. Make sure the login session points at labwc/Pi OS, not
        ...
        if [[ "$theme_used" == "mactahoe" ]]; then
            theme_msg="MacTahoe GTK theme with candy-icons."
        elif [[ "$theme_used" == "skipped" ]]; then
            theme_msg="Adwaita-dark GTK theme with candy-icons (MacTahoe skipped by design)."
        else
...
- Rounded window corners & soft drop shadows
- Circular hover highlight on titlebar buttons
- Previewed Alt-Tab window switcher
- Consistent glass font across titles/menus/OSD
- Glass tooltips & scrollbars
- Themed swaync notifications & swayosd OSD (best-effort)
- No Wayfire required
...
A reboot or logout/login is required to switch sessions." \
    ...
    if [[ "$tier" == "recommended" || "$tier" == "ludicrous" ]]; then
        ...
        ensure_pi_menu_icon || true
        if [[ "$theme_used" == "mactahoe" ]]; then
            theme_msg="Translucent glass theming is active, using the MacTahoe GTK theme with candy-icons."
        else
