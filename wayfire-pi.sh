...
A reboot or logout/login is required to switch sessions." \
    ...
    if [[ $wf_rc -eq 2 ]]; then
        if ui --title "wayfire-pi" --yesno "Apply Minimal mode instead now?\n\n(Standard labwc, no Wayfire, no glass effects — but it works on every Trixie system.)" 10 65; then
            apply_tier "minimal"
        fi
        return 1
...
main_menu() {
    ...
    if ! wayfire_installed && ! wayfire_likely_available; then
        banner="${banner}Note: Wayfire doesn't look available from apt right now — Average and Ludicrous mode may not be installable. Minimal mode is unaffected.\n\n"
