...
# NOTE: pixdecor support (pixdecor_installed/ensure_pixdecor below) is
# currently PARKED — built but not wired into apply_tier/build_wayfire_ini
# anywhere, so it never runs. The only live trace of it left in the
# script is the cleanup line in revert_to_pios, which just removes
# PIXDECOR_PLUGIN_DIR if it happens to exist. Left in place in case
# pixdecor support gets finished later; delete this whole block if it
# doesn't.
PIXDECOR_PLUGIN_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/wayfire/plugin-manager"

pixdecor_installed() {
    if command -v wayfire-plugin >/dev/null 2>&1; then
        wayfire-plugin list 2>/dev/null | grep -qi pixdecor && return 0
    fi
    find / -xdev -iname 'libpixdecor*.so' 2>/dev/null | grep -q . && return 0
    return 1
}

ensure_pixdecor() {
    pixdecor_installed && return 0

    if command -v wayfire-plugin >/dev/null 2>&1; then
        ui --title "wayfire-pi" --infobox "Building pixdecor via wayfire-plugin..." 6 66
        if wayfire-plugin install https://github.com/soreau/pixdecor >/tmp/wayfire-pi-pixdecor-build.log 2>&1; then
            return 0
        fi
        return 1
    fi

    ui --title "wayfire-pi" --infobox "Installing dependencies and compiling pixdecor..." 7 66

    sudo apt-get update >>/tmp/wayfire-pi-pixdecor-build.log 2>&1
    sudo apt-get install -y \
        meson ninja-build pkg-config build-essential git nlohmann-json3-dev \
        wayfire-dev libwf-config-dev libwf-utils-dev libevdev-dev \
        libglm-dev libpixman-1-dev libcairo2-dev libwlroots-0.18-dev \
        >>/tmp/wayfire-pi-pixdecor-build.log 2>&1 || return 1
    ...
    (
        ...
        cd pixdecor
        git checkout 1c38c1d5486e5d72c4e11a0ae517feb2db07ecef || true

        sed -i "s/'>=0.11.0'/'>=0.9.0'/g" meson.build 2>/dev/null || true
        sed -i "s/'wayfire', version: '>=[0-9.]*'/'wayfire', version: '>=0.9.0'/g" meson.build 2>/dev/null || true

        sed -i 's/WLR_AXIS_ORIENTATION_VERTICAL/WL_POINTER_AXIS_VERTICAL_SCROLL/g' src/deco-subsurface.cpp src/decoration.cpp 2>/dev/null || true
        sed -i 's/WLR_AXIS_ORIENTATION_HORIZONTAL/WL_POINTER_AXIS_HORIZONTAL_SCROLL/g' src/deco-subsurface.cpp src/decoration.cpp 2>/dev/null || true

        meson setup build --prefix=/usr
        ninja -C build
        sudo ninja -C build install
    ) >>/tmp/wayfire-pi-pixdecor-build.log 2>&1 || result=1
...
apply_saved_look() {
    ...
    local src="${look_dir}/wayfire.ini"
    if [[ ! -f "$src" ]]; then
        ui --title "wayfire-pi" --msgbox "Couldn't find a saved wayfire.ini for '${picked}'." 8 55
        return 1
    fi
    ensure_wayfire_installed || return 1
    backup_pristine_once
...
revert_to_pios() {
    ...
    # (Do NOT touch active GTK theme files live, as that crashes the running session)
    restore_lightdm_session
    restore_accountsservice_session
    ...
    # 2. Defer ALL session-killing actions (GTK reset, package purges, labwc/panel cleanup) 
    # to the background unit, starting 2 seconds after the user clicks OK.
    local user_home="$HOME"
    sudo systemd-run --unit=wayfire-pi-cleanup --on-active=2s bash -c "
        rm -rf '${PIXDECOR_PLUGIN_DIR}' 2>/dev/null || true
...
main_menu() {
    ...
    choice=$(ui --title "wayfire-pi v${WAYFIRE_PI_VERSION}" --cancel-button "Exit" --menu \
        ...
        "$menu_height" 66 6 \
        1 "Apply the suggested level ($(tier_label "$rec"))" \
    ...
    case "$choice" in
        1) apply_tier "$rec" ;;
        2) apply_tier "minimal" ;;
        3) apply_tier "ludicrous" ;;
        4) revert_to_pios ;;
        5) save_look ;;
        6) apply_saved_look ;;
    esac
